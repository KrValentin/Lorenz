...
int comparaison_RK4(double T, int N, double sigma, double beta, double rho, matrix_t* x0, double epsilon )
{
    ...
    pF_output_X = fopen("stabilite/RK4/comparaison_RK4_X.txt", "w");
    pF_output_Y = fopen("stabilite/RK4/comparaison_RK4_Y.txt", "w");
    pF_output_Z = fopen("stabilite/RK4/comparaison_RK4_Z.txt", "w");
    ...
    resultats = RK4(x0,T, N, sigma, beta, rho);
    set_elem(x0, 0, 0, get_elem(x0, 0,0)+eps);
    resultats_eps = RK4(x0,T, N, sigma, beta, rho);
